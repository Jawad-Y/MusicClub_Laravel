...
import { Guitar, Plus, Pencil, Trash2, Package, UserIcon, Search } from "lucide-react"
...
export default function InstrumentsPage() {
  ...
  const [groupBy, setGroupBy] = useState<"none" | "type" | "condition" | "availability">("none")
  ...
  const isInstrumentAssigned = (instrumentId: number) => {
  ...
  const filteredInstruments = instruments
    .filter((i) => {
      const matchesName =
        ...
        i.unique_code.toLowerCase().includes(nameQuery.toLowerCase())
  ...
  const groupedInstruments = () => {
    if (groupBy === "none") return { "All Instruments": filteredInstruments }
    if (groupBy === "type") {
            ...
            <Dialog open={isAssignDialogOpen} onOpenChange={setIsAssignDialogOpen}>
              ...
              <DialogContent className="sm:max-w-[425px]">
                <form onSubmit={handleAssignSubmit}>
                      ...
                      >
                        ...
                        <SelectContent>
                          {availableInstruments.length === 0 ? (
                            <div className="p-3 text-sm text-muted-foreground">No available instruments</div>
                          ) : (
                  ...
                  <DialogFooter>
                    <Button type="submit" disabled={availableInstruments.length === 0}>
                      Assign Instrument
                    </Button>
                  </DialogFooter>
                </form>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center">
              ...
              <Select value={typeFilter} onValueChange={setTypeFilter}>
                ...
                <SelectContent>
                  <SelectItem value="all">All Types ({instruments.length})</SelectItem>
                  {instrumentTypes.map((t) => (
                    <SelectItem key={t.id} value={t.id.toString()}>
                      {t.name} ({instruments.filter((i) => i.instrument_type_id === t.id).length})
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          <TabsContent value="inventory" className="space-y-4">
            {isLoading ? (
              ...
              (() => {
                ...
                return (
                  <div className="space-y-6">
                    {entries.map(([groupName, groupInstruments]) => (
                      <div key={groupName}>
                        {groupBy !== "none" && (
                          <h3 className="text-lg font-semibold mb-3 text-foreground">
                            {groupName} ({groupInstruments.length})
                          </h3>
                        )}
                                  ...
                                  <div className="flex flex-wrap gap-2">
                                    <span
                                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                                        isInstrumentAssigned(instrument.id)
                                          ? "bg-orange-500/10 text-orange-500 border border-orange-500/20"
                                          : "bg-emerald-500/10 text-emerald-500 border border-emerald-500/20"
                                      }`}
                                    >
                                      {isInstrumentAssigned(instrument.id) ? "Assigned" : "Available"}
                                    </span>
                                    <span
                                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                                        instrument.condition === "excellent"
                                          ...
                                          : instrument.condition === "fair"
                                            ...
                                            : instrument.condition === "needs_repair"
                                            ? "bg-red-500/10 text-red-500 border border-red-500/20"
                                            : instrument.condition === "poor"
                                            ? "bg-red-500/10 text-red-500 border border-red-500/20"
                                            : "bg-blue-500/10 text-blue-500 border border-blue-500/20"
                                      }`}
                                    >
                                      {instrument.condition === "needs_repair" ? "Needs Repair" : instrument.condition.charAt(0).toUpperCase() + instrument.condition.slice(1)}
                                    </span>
                                  </div>
                                  {canManageInventory && (
